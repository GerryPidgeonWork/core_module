...
from gui.G01c_widget_primitives import make_card_label
...
#     • Initialise ttk/ttkbootstrap Style (delegating styling to G01b_style_engine).
...
class BaseGUI(tk.Tk):  # type: ignore[name-defined]
    ...
    Description:
        This class constructs the top-level Tk window, initialises ttk/ttkbootstrap
        styles, and exposes a scrollable main content region. Subclasses override
        build_widgets() to attach their UI to self.main_frame.
    ...
    Args:
        ...
        resizable (bool):
            Whether the user may resize the window horizontally and vertically.
        start_fullscreen (bool):
            If True, the window will enter fullscreen/maximised mode after
            initialisation.
    ...
    Notes:
        - All child widgets must be created under self.main_frame, not directly
          on the root window.
        - Global mouse-wheel scrolling is bound to the canvas; individual
          widgets can override scroll via bind_scroll_widget().
    """
    ...
    def __init__(
        ...
        logger.debug(
            "[G01e] Window request — title=%r width=%d height=%d resizable=%s start_fullscreen=%s",
            title,
            width,
            height,
            resizable,
        ...
        if start_fullscreen:
            ...
            logger.debug("[G01e] start_fullscreen=True → open_fullscreen() called.")
    ...
    # =================================================================================================
    # 3.1 STYLE CONFIGURATION (delegated to G01b_style_engine)
    # =================================================================================================
    def configure_styles(self) -> ttk.Style:  # type: ignore[name-defined]
        ...
        Description:
            Determines an available GUI font family, constructs an appropriate
            Style object (plain ttk or ttkbootstrap), normalises the theme for
            plain ttk, and delegates all visual configuration to
        ...
        Returns:
            ttk.Style:
                The configured style object for this window.

        Raises:
            None.

        Notes:
            - For ttkbootstrap.Style, the bootstrap theme is preserved.
            - For plain ttk.Style, the theme is forced to 'clam' when possible.
            - The chosen font family is stored on self.active_font_family.
        """
    ...
    def build_root_layout(self) -> None:
        ...
        Description:
            Constructs the outer frame, canvas, vertical scrollbar, and the
            scrollable main_frame that all pages attach their widgets to.

        Structure:
            self (Tk)
              └─ self.container (SectionOuter.TFrame)
                   ├─ self.canvas (Canvas)
                   │      └─ window → self.main_frame (SectionBody.TFrame)
        ...
        Notes:
            ...
            - The scrollregion auto-expands as widgets are added.
        """
    ...
    def on_frame_configure(self, event: Any) -> None:
        ...
        Args:
            event (Any):
                Tkinter Configure event (unused but required by binding).
    ...
    def on_canvas_configure(self, event: Any) -> None:
        ...
        Args:
            event (Any):
                Tkinter Configure event containing new width.
    ...
    def bind_global_scroll(self) -> None:
        ...
        Description:
            Binds global scroll events to the root window so that the canvas
            scrolls regardless of which widget currently has focus.
        ...
        Notes:
            ...
            - Linux uses <Button-4>/<Button-5> for scroll up/down.
        """
        os_type = detect_os()
    ...
    # ------------------------------------------------------------------------------------------------
    def on_global_mousewheel(self, event: Any) -> None:
        """
        Scroll the canvas on Windows/macOS.

        Description:
            Normalises the wheel delta and scrolls the canvas in unit steps.

        Args:
            event (Any):
                Tkinter mouse-wheel event with .delta attribute.
        ...
        Notes:
            - Bound globally via bind_global_scroll().
        """
    ...
    # ------------------------------------------------------------------------------------------------
    def on_global_mousewheel_linux(self, event: Any) -> None:
        """
        Scroll the canvas on Linux using Button-4/5 events.

        Description:
            Maps Button-4 to scroll up and Button-5 to scroll down.

        Args:
            event (Any):
                Tkinter Button event with .num attribute.
        ...
        Notes:
            - Bound globally via bind_global_scroll() when OS is Linux.
        """
        if event.num == 4:
    ...
    # =================================================================================================
    def bind_scroll_widget(self, widget: Any) -> None:
        """
        Assign mouse-wheel handling to a specific widget (Text, Listbox, etc.).

        Description:
            When the pointer is over the given widget, scroll events will move
            that widget's own yview instead of the main canvas. Events are
            consumed by returning 'break' from the handlers.

        Args:
            widget (Any):
                Tkinter widget instance that implements .yview().
        ...
        Notes:
            - If the widget has no yview attribute, a warning is logged and the
              call is ignored.
        """
        ...
        def on_widget_mousewheel(event: Any, w: Any = widget) -> str:
            delta_raw = getattr(event, "delta", 0)
            delta = int(delta_raw / 120) if delta_raw else 0
            if delta:
        ...
        def on_widget_mousewheel_linux(event: Any, w: Any = widget) -> str:
            if event.num == 4:
        ...
        if os_type in ("Windows", "MacOS"):
            widget.bind("<MouseWheel>", on_widget_mousewheel, add="+")
        else:
            widget.bind("<Button-4>", on_widget_mousewheel_linux, add="+")
            widget.bind("<Button-5>", on_widget_mousewheel_linux, add="+")
    ...
    def build_widgets(self) -> None:
        """
        Populate the GUI with content — subclasses MUST override.

        Description:
            Hook method called during __init__ after the scroll engine is built.
            The base implementation does nothing. Concrete windows must provide
            their own implementation and attach widgets to self.main_frame.
        ...
        Notes:
            - All widgets must be children of self.main_frame.
    ...
    def center_window(self, width: int, height: int) -> None:
        ...
        Description:
            Calculates screen dimensions and positions the window so that it is
        ...
        Notes:
            - Uses update_idletasks() to ensure geometry information is fresh.
        """
    ...
    def open_fullscreen(self) -> None:
        ...
        Description:
            Uses platform-specific techniques to maximise the window:
                - Windows: state('zoomed')
                - macOS/Linux: attributes('-fullscreen', True)
    ...
    def exit_fullscreen(self) -> None:
        ...
        Description:
            Reverses open_fullscreen(), restoring normal window state.
        ...
        Notes:
            - On macOS/Linux, clears the -fullscreen attribute.
            - On Windows, resets the window state to 'normal'.
        """
        os_type = detect_os()
    ...
    def safe_close(self) -> None:
        ...
        Description:
            Convenience hook for subclasses to override if cleanup work is
            required before the window is destroyed.
        ...
        Notes:
            - Default implementation simply calls destroy().
        """
    ...
    def close(self) -> None:
        ...
        Description:
            Provides a short, semantic alias for safe_close() so callers can
        ...
        Notes:
            - Subclasses should override safe_close(), not this method.
        """
        self.safe_close()
    ...
    def log_debug_diagnostics(self) -> None:
        ...
        Description:
            Logs Tk version, active theme, available themes, active font family,
            and the existence of several critical ttk styles.
        ...
        Notes:
            - Controlled by DEBUG_BASE_GUI flag at module level.
        """
        try:
...
# 4. SANDBOX TEST
# ----------------------------------------------------------------------------------------------------
# A simple frame demonstrating BaseGUI behaviour using G01c primitives.
# ====================================================================================================
if __name__ == "__main__":
    init_logging()
    ...
    class G01eTestGUI(BaseGUI):
        ...
        def build_widgets(self) -> None:
            """
            Description:
                Populate the sandbox window with headings, body lines, and a
                scrollable console area to exercise BaseGUI behaviour.
            ...
            Notes:
                - All widgets are attached to self.main_frame.
            """
            # Window heading
            make_label(
                self.main_frame,
